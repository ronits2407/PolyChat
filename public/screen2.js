...
async function getanswer(promptInfo) {
  ...
  try {
    ...
    if (!promptInfo.chatId) {
      const chatTitleTemplate = document
        .querySelector(".recentChat")
        .cloneNode(true);
      chatTitleTemplate.style.display = "flex";
      let link = chatTitleTemplate.querySelector("a");
      link.textContent = message;
      link.href = `${window.location.origin}/screen2.html?chatid=${encodeURIComponent(data.chatId)}`

      recentChatList.prepend(chatTitleTemplate);
    }
    document.querySelectorAll(".recentChat")[1].querySelector("a").id =
      data.chatId;
    currId = data.chatId;
